    ...
    <div v-if="binlog.show" class="tl-overlay" @click.self="binlog.show = false">
      <div class="tl-modal">
        ...
        <div class="tl-modal-body">
          ...
          <p class="tl-note">
            <b>Бинлог по умолчанию выключен</b> — тогда команда ниже вернёт пустоту.
            Он нужен не всегда: кто нажал, видно в этом журнале, а что изменилось через API —
            в логе gtsAPI. Включают его точечно, под конкретное расследование:
            как — в README пакета, разделы для OpenServer и Ubuntu.
          </p>
          <p class="tl-note">
            Команду выполнить на сервере БД — она покажет строки, изменённые именно этим
            запросом (по соединению MySQL и окну времени).
          </p>
