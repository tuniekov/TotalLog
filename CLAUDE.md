...
В конфиг они попадают триггером `gtsapi_rule` (`TotalLog::ruleTLItem`, регистрируется через
`regTriggers()`; `loadModels: 'totallog'` в свойствах таблицы). Триггер нужен по двум причинам:

- gtsAPI сам подставляет такие поля только в таблицу, зарегистрированную в `gtsAPIFieldTable`
  (у нас `TLItem`) — `TLItemUser` осталась бы без них;
- подставляет он их редактируемыми, а журнал только читают → триггер ставит `readonly`.
