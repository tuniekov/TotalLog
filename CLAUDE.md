...
Журнал отвечает «кто нажал». Что реально изменилось в базе — знает бинлог (ROW): он пишет
каждую изменённую строку, включая правки мимо API.

**Постоянно он не нужен** (решено 14.08.2026): legacy-код всё равно поднимает MODX и попадает
в TotalLog, а построчные изменения через API пишет `gtsAPILog` с `data_before`/`data_after`
и откатом `restore_version`. Мимо обоих идут только ручные правки SQL. Бинлог остаётся
инструментом точечного расследования — включил, поймал, выключил.

Связка готова: `thread_id` + окно `created_at … finished_at` в каждой записи. Кнопка
«Изменения в БД» собирает из них команду `mysqlbinlog` и честно предупреждает, что при
выключенном бинлоге вывод будет пустым.

⚠️ `expire_logs_days = 0` (значение по умолчанию) означает, что логи не чистятся никогда.
Инструкции по включению для OSPanel/OpenServer и Ubuntu (MySQL 5.7) — в `README.md`.
